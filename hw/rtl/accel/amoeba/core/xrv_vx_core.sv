...
module xrv_vx_core import amoeba_gpu_pkg::*; #(
    parameter CORE_ID               = 0,
...
`ifdef PERF_ENABLE
    ...
    for (genvar i = 0; i < NUM_LSU_BLOCKS_P; ++i) begin : g_perf_dcache
        for (genvar j = 0; j < NUM_LSU_LANES_P; ++j) begin : g_j
            assign perf_dcache_rd_req_fire[i * NUM_LSU_LANES_P + j] = lsu_mem_if[i].req_valid && lsu_mem_if[i].req_data.mask[j] && lsu_mem_if[i].req_ready && ~lsu_mem_if[i].req_data.rw;
            assign perf_dcache_wr_req_fire[i * NUM_LSU_LANES_P + j] = lsu_mem_if[i].req_valid && lsu_mem_if[i].req_data.mask[j] && lsu_mem_if[i].req_ready && lsu_mem_if[i].req_data.rw;
            assign perf_dcache_rsp_fire[i * NUM_LSU_LANES_P + j] = lsu_mem_if[i].rsp_valid && lsu_mem_if[i].rsp_data.mask[j] && lsu_mem_if[i].rsp_ready;
        end
    end
