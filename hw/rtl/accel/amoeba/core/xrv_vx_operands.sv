...
`endif
    ...
    localparam META_DATAW = ISSUE_WIS_W + NUM_THREADS_P + PC_WIDTH_P + 1 + VX_EX_BITS + VX_INST_OP_BITS + VX_INST_ARGS_BITS + VX_NR_BITS + UUID_WIDTH_P;
    localparam REGS_DATAW = XLEN_P * NUM_THREADS_P;
    localparam DATAW = META_DATAW + NUM_SRC_OPDS * REGS_DATAW;
    localparam RAM_ADDRW = `XM_LOG2UP(VX_NUM_REGS * PER_ISSUE_WARPS);
    ...
    wire [NUM_BANKS_P-1:0] gpr_rd_valid, gpr_rd_ready;
    wire [NUM_BANKS_P-1:0] gpr_rd_valid_st1, gpr_rd_valid_st2;
    ...
    assign gpr_rd_ready = {NUM_BANKS_P{pipe_ready_in}};
    ...
    always @(posedge clk_i) begin
        if (rst_i || scoreboard_if.ready) begin
            data_fetched_st1 <= 0;
        end else begin
...
`ifdef PERF_ENABLE
    ...
    always @(posedge clk_i) begin
        if (rst_i) begin
            ...
            collisions_r <= collisions_r + VX_PERF_CTR_BITS'(scoreboard_if.valid && pipe_ready_in && has_collision_n);
        end
    end
