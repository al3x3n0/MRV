...
`endif
    ...
    input wire [VX_PERF_CTR_BITS-1:0]                 cycles,
    input wire [NUM_WARPS_P-1:0]                    active_warps,
    input wire [NUM_WARPS_P-1:0][NUM_THREADS_P-1:0]  thread_masks,

    input wire                          read_enable,
    input wire [UUID_WIDTH_P-1:0]       read_uuid,
    input wire [WID_WIDTH_P-1:0]        read_wid,
    input wire [VX_CSR_ADDR_BITS-1:0]   read_addr,
    output wire [XLEN_P-1:0]            read_data_ro,
    output wire [XLEN_P-1:0]            read_data_rw,

    input wire                          write_enable,
    input wire [UUID_WIDTH_P-1:0]       write_uuid,
    input wire [WID_WIDTH_P-1:0]        write_wid,
    input wire [VX_CSR_ADDR_BITS-1:0]   write_addr,
    input wire [XLEN_P-1:0]             write_data
);

    `XM_UNUSED_VAR (rst_i)
    `XM_UNUSED_VAR (write_wid)
    `XM_UNUSED_VAR (write_data)

    // CSRs Write /////////////////////////////////////////////////////////////

    reg [XLEN_P-1:0] mscratch;

`ifdef EXT_F_ENABLE
    reg [NUM_WARPS_P-1:0][VX_INST_FRM_BITS+`FP_FLAGS_BITS-1:0] fcsr, fcsr_n;
    wire [NUM_FPU_BLOCKS_P-1:0]              fpu_write_enable;
    wire [NUM_FPU_BLOCKS_P-1:0][WID_WIDTH_P-1:0] fpu_write_wid;
    fflags_t [NUM_FPU_BLOCKS_P-1:0]          fpu_write_fflags;

    for (genvar i = 0; i < NUM_FPU_BLOCKS_P; ++i) begin : g_fpu_write
    ...
    always @(*) begin
        fcsr_n = fcsr;
        for (integer i = 0; i < NUM_FPU_BLOCKS_P; ++i) begin
        ...
        if (write_enable) begin
            case (write_addr)
                ...
                VX_CSR_FRM:    fcsr_n[write_wid][VX_INST_FRM_BITS+`FP_FLAGS_BITS-1:`FP_FLAGS_BITS] = write_data[VX_INST_FRM_BITS-1:0];
                VX_CSR_FCSR:   fcsr_n[write_wid] = write_data[`FP_FLAGS_BITS+VX_INST_FRM_BITS-1:0];
            default:;
            endcase
        end
    end

    for (genvar i = 0; i < NUM_FPU_BLOCKS_P; ++i) begin : g_fpu_csr_read_frm
        assign fpu_csr_if[i].read_frm = fcsr[fpu_csr_if[i].read_wid][VX_INST_FRM_BITS+`FP_FLAGS_BITS-1:`FP_FLAGS_BITS];
    end
...
`endif
    ...
    reg [XLEN_P-1:0] read_data_ro_w;
    reg [XLEN_P-1:0] read_data_rw_w;
    ...
    always @(*) begin
        ...
        `ifdef EXT_F_ENABLE
            ...
            VX_CSR_FCSR       : read_data_rw_w = XLEN_P'(fcsr[read_wid]);
        `endif
            VX_CSR_MSCRATCH   : read_data_rw_w = mscratch;

            VX_CSR_WARP_ID    : read_data_ro_w = XLEN_P'(read_wid);
            VX_CSR_CORE_ID    : read_data_ro_w = XLEN_P'(CORE_ID);
            VX_CSR_ACTIVE_THREADS: read_data_ro_w = XLEN_P'(thread_masks[read_wid]);
            VX_CSR_ACTIVE_WARPS: read_data_ro_w = XLEN_P'(active_warps);
            VX_CSR_NUM_THREADS: read_data_ro_w = XLEN_P'(NUM_THREADS_P);
            VX_CSR_NUM_WARPS  : read_data_ro_w = XLEN_P'(NUM_WARPS_P);
            // FIXME VX_CSR_NUM_CORES  : read_data_ro_w = XLEN_P'(`NUM_CORES * `NUM_CLUSTERS);
            // FIXME VX_CSR_LOCAL_MEM_BASE: read_data_ro_w = XLEN_P'(`LMEM_BASE_ADDR);
            ...
            VX_CSR_MEPC,
            VX_CSR_PMPCFG0,
            VX_CSR_PMPADDR0 : read_data_ro_w = XLEN_P'(0);
            ...
            default: begin
                ...
                `ifdef PERF_ENABLE
                    ...
                    `else
                        `CSR_READ_64(VX_CSR_MPM_SCRB_FPU, read_data_ro_w, VX_PERF_CTR_BITS'(0));
                    `endif
...
`ifdef PERF_ENABLE
    `XM_UNUSED_VAR (mem_perf_if.icache);
    `XM_UNUSED_VAR (mem_perf_if.lmem);
`endif
