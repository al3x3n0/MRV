...
module xrv_vx_csr_unit import xrv_vx_gpu_pkg::*; #(
...
`endif
    ...
    wire [`xrv_vx_CSR_ADDR_BITS-1:0] csr_addr = execute_if.data.op_args.csr.addr;
    wire [`NRI_BITS-1:0] csr_imm = execute_if.data.op_args.csr.imm;

    wire is_fpu_csr = (csr_addr <= `xrv_vx_CSR_FCSR);
    ...
    always @(*) begin
        csr_rd_enable = 0;
        case (csr_addr)
        `xrv_vx_CSR_THREAD_ID : csr_read_data = wtid;
        `xrv_vx_CSR_MHARTID   : csr_read_data = gtid;
        default : begin
