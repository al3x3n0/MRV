...
module xrv_vx_scoreboard import xrv_vx_gpu_pkg::*; #(
...
`ifdef PERF_ENABLE
    ...
    output reg [NUM_EX_UNITS_P-1:0][`PERF_CTR_BITS-1:0] perf_units_uses,
    output reg [NUM_SFU_UNITS_P-1:0][`PERF_CTR_BITS-1:0] perf_sfu_uses,
`endif
    ...
    localparam DATAW = UUID_WIDTH_P + NUM_THREADS_P + PC_WIDTH_P + `EX_BITS + `INST_OP_BITS + `INST_ARGS_BITS + (`NR_BITS * 4) + 1;

    xrv_vx_ibuffer_if staging_if [PER_ISSUE_WARPS]();
    reg [PER_ISSUE_WARPS-1:0] operands_ready;

`ifdef PERF_ENABLE
    reg [PER_ISSUE_WARPS-1:0][NUM_EX_UNITS_P-1:0] perf_inuse_units_per_cycle;
    wire [NUM_EX_UNITS_P-1:0] perf_units_per_cycle, perf_units_per_cycle_r;

    reg [PER_ISSUE_WARPS-1:0][NUM_SFU_UNITS_P-1:0] perf_inuse_sfu_per_cycle;
    wire [NUM_SFU_UNITS_P-1:0] perf_sfu_per_cycle, perf_sfu_per_cycle_r;

    xrv_reduce #(
        .DATAW_IN (NUM_EX_UNITS_P),
        .N  (PER_ISSUE_WARPS),
        .OP ("|")
    ) perf_units_reduce (
    ...
    for (genvar i = 0; i < NUM_EX_UNITS_P; ++i) begin : g_perf_units_uses
    ...
    for (genvar i = 0; i < NUM_SFU_UNITS_P; ++i) begin : g_perf_sfu_uses
        always @(posedge clk_i) begin
            if (rst_i) begin
                perf_sfu_uses[i] <= '0;
            end else begin
