...
module xrv_vx_dispatch_unit import xrv_vx_gpu_pkg::*; #(
    ...
    // inputs
    xrv_vx_dispatch_if.slave    dispatch_if [`ISSUE_WIDTH],
    ...
    `STATIC_ASSERT (`IS_DIVISBLE(NUM_THREADS_P, NUM_LANES), ("invalid parameter"))
    ...
    localparam IN_DATAW     = UUID_WIDTH_P + ISSUE_WIS_W + NUM_THREADS_P + `INST_OP_BITS + `INST_ARGS_BITS + 1 + `PC_BITS + `NR_BITS + `NT_WIDTH + (3 * NUM_THREADS_P * XLEN_P);
    localparam OUT_DATAW    = UUID_WIDTH_P + `NW_WIDTH + NUM_LANES + `INST_OP_BITS + `INST_ARGS_BITS + 1 + `PC_BITS + `NR_BITS + `NT_WIDTH + (3 * NUM_LANES * XLEN_P);
    ...
    for (genvar block_idx = 0; block_idx < BLOCK_SIZE; ++block_idx) begin : g_blocks
        ...
        wire [`NW_WIDTH-1:0] block_wid = wis_to_wid(dispatch_data[issue_idx][DATA_TMASK_OFF+NUM_THREADS_P +: ISSUE_WIS_W], isw);
