...
module xrv_vx_issue import xrv_vx_gpu_pkg::*; #(
...
`endif
    ...
    xrv_vx_dispatch_if.master   dispatch_if [NUM_FU_P * ISSUE_WIDTH_P]
);
    `STATIC_ASSERT ((ISSUE_WIDTH_P <= `NUM_WARPS), ("invalid parameter"))

`ifdef PERF_ENABLE
    issue_perf_t per_issue_perf [ISSUE_WIDTH_P];
    `PERF_COUNTER_ADD (issue_perf, per_issue_perf, ibf_stalls, `PERF_CTR_BITS, ISSUE_WIDTH_P, (ISSUE_WIDTH_P > 2))
    `PERF_COUNTER_ADD (issue_perf, per_issue_perf, scb_stalls, `PERF_CTR_BITS, ISSUE_WIDTH_P, (ISSUE_WIDTH_P > 2))
    `PERF_COUNTER_ADD (issue_perf, per_issue_perf, opd_stalls, `PERF_CTR_BITS, ISSUE_WIDTH_P, (ISSUE_WIDTH_P > 2))
    for (genvar i = 0; i < NUM_FU_P; ++i) begin : g_issue_perf_units_uses
        `PERF_COUNTER_ADD (issue_perf, per_issue_perf, units_uses[i], `PERF_CTR_BITS, ISSUE_WIDTH_P, (ISSUE_WIDTH_P > 2))
    end
    for (genvar i = 0; i < `NUM_SFU_UNITS; ++i) begin : g_issue_perf_sfu_uses
        `PERF_COUNTER_ADD (issue_perf, per_issue_perf, sfu_uses[i], `PERF_CTR_BITS, ISSUE_WIDTH_P, (ISSUE_WIDTH_P > 2))
    end
`endif
    ...
    for (genvar issue_id = 0; issue_id < ISSUE_WIDTH_P; ++issue_id) begin : g_slices
        ...
        xrv_vx_dispatch_if per_issue_dispatch_if[NUM_FU_P]();
    ...
    `endif
        ...
        for (genvar ex_id = 0; ex_id < NUM_FU_P; ++ex_id) begin : g_dispatch_if
            `ASSIGN_xrv_vx_IF(dispatch_if[ex_id * ISSUE_WIDTH_P + issue_id], per_issue_dispatch_if[ex_id]);
        end
     end
