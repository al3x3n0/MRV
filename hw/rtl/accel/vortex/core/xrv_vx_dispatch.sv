...
module xrv_vx_dispatch import xrv_vx_gpu_pkg::*; #(
...
`endif
    ...
    // outputs
    xrv_vx_dispatch_if.master   dispatch_if [`NUM_EX_UNITS]
);
    `UNUSED_SPARAM (INSTANCE_ID)

    localparam DATAW = UUID_WIDTH_P + ISSUE_WIS_W + NUM_THREADS_P + PC_WIDTH_P + `INST_OP_BITS + `INST_ARGS_BITS + 1 + RF_ADDR_WIDTH_P + (3 * NUM_THREADS_P * `XLEN) + TID_WIDTH_LP;
