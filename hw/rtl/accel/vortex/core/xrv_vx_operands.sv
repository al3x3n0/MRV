...
`endif
    ...
    localparam PER_BANK_REGS = `NUM_REGS / NUM_BANKS_P;
    localparam META_DATAW = ISSUE_WIS_W + NUM_THREADS_P + PC_WIDTH_P + 1 + `EX_BITS + `INST_OP_BITS + `INST_ARGS_BITS + `NR_BITS + UUID_WIDTH_P;
    localparam REGS_DATAW = XLEN_P * NUM_THREADS_P;
    localparam DATAW = META_DATAW + NUM_SRC_OPDS * REGS_DATAW;
    localparam RAM_ADDRW = `XM_LOG2UP(`NUM_REGS * PER_ISSUE_WARPS);
