...
`ifdef EXT_F_ENABLE
    reg [NUM_WARPS_P-1:0][`INST_FRM_BITS+`FP_FLAGS_BITS-1:0] fcsr, fcsr_n;
...
`endif
    ...
    always @(*) begin
        ...
        read_addr_valid_w = 1;
        case (read_addr)
            `VX_CSR_MVENDORID  : read_data_ro_w = XLEN_P'(`VENDOR_ID);
            `VX_CSR_MARCHID    : read_data_ro_w = XLEN_P'(`ARCHITECTURE_ID);
            `VX_CSR_MIMPID     : read_data_ro_w = XLEN_P'(`IMPLEMENTATION_ID);
            `VX_CSR_MISA       : read_data_ro_w = XLEN_P'({2'(`CLOG2(XLEN_P/16)), 30'(`MISA_STD)});
        `ifdef EXT_F_ENABLE
            `VX_CSR_FFLAGS     : read_data_rw_w = XLEN_P'(fcsr[read_wid][`FP_FLAGS_BITS-1:0]);
            `VX_CSR_FRM        : read_data_rw_w = XLEN_P'(fcsr[read_wid][`INST_FRM_BITS+`FP_FLAGS_BITS-1:`FP_FLAGS_BITS]);
        ...
        `endif
            ...
            `VX_CSR_NUM_CORES  : read_data_ro_w = XLEN_P'(`NUM_CORES * `NUM_CLUSTERS);
            `VX_CSR_LOCAL_MEM_BASE: read_data_ro_w = XLEN_P'(`LMEM_BASE_ADDR);
