...
module xrv_vx_commit #(
    ...
    localparam DATA_WIDTH_P = UUID_WIDTH_P + WID_WIDTH_P + NUM_THREADS_P + PC_WIDTH_P + 1 + `NR_BITS + NUM_THREADS_P * XLEN_P + 1 + 1 + 1;
    localparam COMMIT_SIZEW = `CLOG2(NUM_THREADS_P + 1);
    ...
    for (genvar i = 0; i < ISSUE_WIDTH_P; ++i) begin : g_commit_arbs
        ...
        wire [NUM_EX_UNITS_P-1:0]            valid_in;
        wire [NUM_EX_UNITS_P-1:0][DATA_WIDTH_P-1:0] data_in;
        wire [NUM_EX_UNITS_P-1:0]            ready_in;

        for (genvar j = 0; j < NUM_EX_UNITS_P; ++j) begin : g_data_in
            assign valid_in[j] = commit_if[j * ISSUE_WIDTH_P + i].valid;
            assign data_in[j]  = commit_if[j * ISSUE_WIDTH_P + i].data;
            assign commit_if[j * ISSUE_WIDTH_P + i].ready = ready_in[j];
        end
...
`ifdef DBG_TRACE_PIPELINE
    for (genvar i = 0; i < ISSUE_WIDTH_P; ++i) begin : g_trace
        for (genvar j = 0; j < NUM_EX_UNITS_P; ++j) begin : g_j
            always @(posedge clk_i) begin
                if (commit_if[j * ISSUE_WIDTH_P + i].valid && commit_if[j * ISSUE_WIDTH_P + i].ready) begin
