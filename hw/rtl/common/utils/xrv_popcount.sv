...
module xrv_popcount #(
    parameter MODEL = 1,
    parameter N     = 1,
    parameter M     = `XM_CLOG2(N+1)
) (
    input  logic [N-1:0] data_in,
    output logic [M-1:0] data_out
);
    `XM_UNUSED_PARAM (MODEL)
...
`else
    if (N == 1) begin : g_passthru
        ...
        xrv_popcount32 pc32(t_in, t_out);
        ...
        xrv_popcount63 pc63(t_in, t_out);
        assign data_out = t_out[M-1:0];
        ...
        xrv_popcount63 pc63(t_in[5:0], t1_out[2:0]);
        xrv_popcount32 pc32(t_in[8:6], t1_out[4:3]);
        xrv_sum33 sum33(t1_out[2:0], {1'b0, t1_out[4:3]}, t2_out);
        ...
        xrv_popcount63 pc63a(t_in[5:0],  t1_out[2:0]);
        xrv_popcount63 pc63b(t_in[11:6], t1_out[5:3]);
        xrv_sum33 sum33(t1_out[2:0], t1_out[5:3], t2_out);
        assign data_out = t2_out[M-1:0];
        ...
        xrv_popcount63 pc63a(t_in[5:0],   t1_out[2:0]);
        xrv_popcount63 pc63b(t_in[11:6],  t1_out[5:3]);
        xrv_popcount63 pc63c(t_in[17:12], t1_out[8:6]);
        xrv_popcount32 pc32a({t1_out[0], t1_out[3], t1_out[6]}, t2_out[1:0]);
        xrv_popcount32 pc32b({t1_out[1], t1_out[4], t1_out[7]}, t2_out[3:2]);
        xrv_popcount32 pc32c({t1_out[2], t1_out[5], t1_out[8]}, t2_out[5:4]);
        assign data_out = {2'b0,t2_out[1:0]} + {1'b0,t2_out[3:2],1'b0} + {t2_out[5:4],2'b0};

    end else if (MODEL == 1) begin : g_model1

        localparam PN = 1 << `XM_CLOG2(N);
        localparam LOGPN = `XM_CLOG2(PN);

    `IGNORE_UNOPTFLAT_BEGIN
        logic [M-1:0] tmp [LOGPN-1:0][PN-1:0];
