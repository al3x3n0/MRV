...
`endif
    ...
    if (BYPASS_ENABLE_LP) begin : g_bypass
        ...
        xrv_cache_bypass #(
            ...
            .CACHE_ENABLED_P    (!PASSTHRU_P),

            .WORD_SIZE_P        (WORD_SIZE_P),
            .LINE_SIZE_P        (LINE_SIZE_P),

            .CORE_ADDR_WIDTH_P  (CACHE_WORD_ADDR_WIDTH_LP),
            .CORE_TAG_WIDTH_P   (TAG_WIDTH_P),

            .CACHE_MEM_ADDR_WIDTH_P     (CACHE_MEM_ADDR_WIDTH_LP),
            .CACHE_MEM_TAG_IN_WIDTH_P   (CACHE_MEM_TAG_WIDTH_P),
...
`ifdef DBG_TRACE_CACHE
    for (genvar i = 0; i < NUM_REQS_P; ++i) begin : g_trace_core
        always @(posedge clk_i) begin
            if (core_bus_if[i].req_vld && core_bus_if[i].req_rdy) begin
            ...
            if (core_bus_if[i].resp_vld && core_bus_if[i].resp_rdy) begin
                `TRACE(2, ("%t: %s core-rd-resp[%0d]: tag=0x%0h, req_idx=%0d, data=0x%h (#%0d)\n", $time, INSTANCE_ID, i, core_bus_if[i].resp_data.tag.value, i, core_bus_if[i].resp_data.data, core_bus_if[i].resp_data.tag.uuid))
            end
        end
    end
    ...
    for (genvar i = 0; i < NUM_MEM_PORTS_P; ++i) begin : g_trace_mem
        always @(posedge clk_i) begin
            if (mem_bus_if[i].req_vld && mem_bus_if[i].req_rdy) begin
            ...
            if (mem_bus_if[i].resp_vld && mem_bus_if[i].resp_rdy) begin
                `TRACE(2, ("%t: %s mem-rd-resp[%0d]: data=0x%h, tag=0x%0h (#%0d)\n",
                    $time, INSTANCE_ID, i, mem_bus_if[i].resp_data.data, mem_bus_if[i].resp_data.tag.value, mem_bus_if[i].resp_data.tag.uuid))
            end
        end
    end
`endif
